...
- `SCP BATCH FILE {json}`：每个文件的 start、complete 或 failed 事件，包含 operation id、序号、总数、远端/本地路径、大小和错误。
- `SCP BATCH PROGRESS ...`：每次完成后的累计进度，进程被外部终止时仍能找到最后完成数。
- `SSH EXECUTE {json}`：内部 BusyBox 枚举和 `wc` 查询的完整结果，但不进入控制台。
...
- 聚焦回归：`114 passed, 1 deselected`；deselect 的项目是下述主线既有失败。
- 统一环境脚本契约：`9 passed`；Tool 验证、注册发现、四个仓库 Skill 校验、Python 编译和 JavaScript 语法检查均通过。
- 全量离线 UT：`283 passed, 1 failed`。唯一失败为 `test_selectors_reject_unsafe_paths_and_package_ambiguity` 在 POSIX 上未拒绝 Windows 盘符路径；已在干净的 `origin/main` 单独复现，不是本功能引入。
- 排除该既有基线项后：`283 passed, 1 deselected`。
