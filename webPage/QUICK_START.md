...
只有成功 `finalize()` 的批次会出现在查询列表。可打开多个目标日志窗口，按文件顺序分页浏览，或填写中心时间和可选日期进行共享时间窗查询；窗口分钟数按中心前后各一半解释。每个窗口的 `Find` 输入框会在整个 target file 中执行不区分大小写的关键词筛选，“上下 N 行”允许 `0..50`，默认 `3`。查询先对命中分页，再完整展开本页每个命中的上下文，因此上下文不会被普通分页边界截断；重叠窗口会去重并保持原始顺序。命中行显示暖色并高亮关键词，上下文行显示蓝色；时间窗只筛命中行。点击有时间的记录后，其他窗口中相差不超过五分钟的已显示记录会高亮；完全没有时间的 `[-]` 和日期文本非法的 `[?]` 记录只按原始顺序浏览，不参与时间查询和联动。
