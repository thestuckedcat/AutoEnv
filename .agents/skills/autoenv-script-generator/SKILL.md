...
- For the confirmed log-collection use case, use the public `ctx.create_log_collection()`, `collection.download()/extract_all()/group()/finalize()`, and matcher APIs. Use `MetadataPattern` named `slot_id`/`socket_id` groups for inheritable attributes and `match_block(boundary_mode="strict", consume_regex=...)` for confirmed minimal blocks; keep the legacy default when old whole-block timestamp behavior is required. Keep workflow-only log collection in `webPage/tools/`.
