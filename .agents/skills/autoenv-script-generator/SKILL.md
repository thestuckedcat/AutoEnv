...
- For the confirmed log-collection use case, use the public `ctx.create_log_collection()`, `collection.download()/extract_all()/group()/finalize()`, and `group.match_line()/match_block()` APIs. Keep workflow-only log collection in `webPage/tools/`; do not expose it as an environment-startup script.
