...
def test_script_metadata_is_inferred_from_package_and_connection_calls(tmp_path):
    root = _project_root(tmp_path)

    @register_script("automatic_metadata")
    def automatic_metadata(ctx: RunContext):
        firmware = package(
            "firmware",
            alias="主固件包",
            description="从 HDFS 下载的升级固件。",
    ...
    definition = get_script("automatic_metadata", root_dir=root)
    assert definition.packages == ("firmware",)
    assert definition.package_inputs == ({
        "name": "firmware",
        "alias": "主固件包",
...
def test_automatic_metadata_defaults_alias_to_name_and_description_to_empty(tmp_path):
    root = _project_root(tmp_path)

    @register_script("default_metadata")
    def default_metadata(ctx: RunContext):
        firmware = package("firmware")
        ctx.register_ssh_host("dut", resource_label="1260网口")
        ctx.argument("ordinary_business_input")
        return ctx.download_package(firmware)

    definition = get_script("default_metadata", root_dir=root)
    assert definition.package_inputs == ({
        "name": "firmware",
        "alias": "firmware",
        "description": "",
    ...
    assert definition.resources[0]["description"] == ""
