...
| `test_ssh_host.py` | fake Paramiko/SFTP/SCP/远端文件系统 | SSH 状态、连接复用、按输出响应、单/批量传输、BusyBox 无 `find`/无 `wc` 枚举、四路并行峰值、稳定结果顺序、远端 mtime、日志批量无大小比对、批量进度/静默逐文件明细、部分失败清理、上传校验、占位符目标 | `autoenv/ssh_host.py` 和 fake 收到的命令/响应/文件 |
