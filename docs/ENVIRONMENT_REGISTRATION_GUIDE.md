...
仓库还提供可复制的 [`../scripts/template.py`](../scripts/template.py)，集中展示全部
公共选择器、连接、传输、命令、结果判断和组合方式，并在模块说明与装饰器旁标明
每项如何映射到 Web“环境启动”页面。该模板使用占位地址，默认只用于阅读和复制。
...
if not result.success:
    return result
result = groups["product"].match_line_block(
    r"\[ERROR\]",
    "errors.log",
)
if not result.success:
